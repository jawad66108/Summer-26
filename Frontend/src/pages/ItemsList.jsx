...
export default function ItemsList() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    getItems(filters)
      .then((data) => { if (!cancelled) setItems(Array.isArray(data) ? data : data.items || []) })
      .catch(() => { if (!cancelled) setError('Could not load items.') })
  ...
  return (
    <Layout>
      <div className="topbar">
        ...
        {isAdmin && (
          <button className="btn btn-primary display" onClick={() => navigate('/items/new')}>
            + Add Item
          </button>
        )}
      </div>
      ...
      <div className="panel">
        ...
        {!loading && !error && items.length > 0 && (
          <table>
            <thead>
              <tr>
                <th>Item</th><th>Category</th><th>Sport</th><th>Unit</th><th>On Hand</th><th>Status</th>
                {isAdmin && <th></th>}
              </tr>
            </thead>
            <tbody>
              {items.map((item) => (
                <tr key={item.id} className="data-row" onClick={() => navigate(`/items/${item.id}`)}>
                  <td>
                    <div className="row-with-thumb">
                      {item.photoUrl || item.photo_url ? (
                        <img src={item.photoUrl || item.photo_url} alt="" className="row-thumb" />
                      ) : (
                        <div className="row-thumb-placeholder">NO<br/>PHOTO</div>
                      )}
                      <b>{item.name}</b>
                    </div>
                  </td>
                  ...
                  <td><Badge status={statusFor(item)} /></td>
                  {isAdmin && (
                    <td>
                      <button
